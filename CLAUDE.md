...
- **Dev & ship workflow (2026-07-24).** Full runbook: `docs/engineering/dev-workflow.md`. Stack is settled — **Bun/Railway relay, no Next.js** (the core is a WebSocket voice relay; serverless can't hold it); interactive UI arrives later as `apps/studio` (Vite+React). **Branch → PR → CI → merge → CD** now: CI (`ci.yml`) gates PRs on `bun run check`; CD (`deploy.yml`) runs check → migrate → `railway up` on merge to main. No branch protection (free GitHub) — **solo-merge discipline** instead: Sven is the only merger and merges only on green CI; `deploy.yml` re-runs `check` so a red main never actually deploys. **Docs & `.claude` tooling ride the currently open work branch** (they don't deploy; never a standalone parked branch) — if no branch is open, short branch + immediate docs-only PR merged on green CI; `deploy.yml` `paths-ignore`s `docs/**`/`**/*.md`/`.claude/**`. PRs+CI gate deployable code. Local dev = **local** Supabase + `bun run dev:all` (guards against dev-on-prod, boots the bridge) — covers Supabase + web + HTTP logic + tests; the tunnel + separate dev Twilio/OpenAI accounts are deferred (2026-07-24), so live-call validation is against prod after deploy. Two envs, strictly separate: `.env.local` (local) vs Railway vars (prod).
- **Keep Linear current — always** (migrated 2026-07-24). Work is tracked in the **Elsewhere** Linear team (`ETC-*`) via the `linear-server` MCP: grouped labels (`area`/`world`/`type` + `needs:sven` = the decision queue), Projects = deliverables (*World Zero — Switchboard*, *The Underworld — Devil*, *Stage 0 — Prove & Launch*). Update tickets as part of any work — state changes, new tasks as they arise, decision-queue accuracy — in the SAME change set as the work, not as an afterthought. This is non-optional. `docs/tracker.md` is frozen pre-migration history (its header maps old items → ETC tickets); never add to it. **Decisions/canon/specs always stay in git docs** — a ticket references a doc, never replaces it. Ticket format is enforced by the `ticket-spec` + `ticket-verification` skills. Structure + view recipes: `docs/engineering/dev-workflow.md` §"Work tracking — Linear".
