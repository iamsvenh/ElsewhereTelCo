...
- **Strict linting from day one, zero tolerance (2026-07-24).** `eslint.config.mjs` (adapted from Borker). `@typescript-eslint/no-explicit-any` is an ERROR — type wire events, never `any` them. Plus strict TS rules, no-non-null-assertion, `consistent-type-imports`, `curly`/`eqeqeq`/`prefer-const`/`no-var`. We keep it at zero (Borker waited and carries a ~1500 backlog). `no-console` is off — the bridge is a server, stdout is observability. `bun run lint` (in `check` + CI).
