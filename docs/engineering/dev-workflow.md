...
**Docs & tooling ride the open work branch (Sven, 2026-07-24; revised same day).** PRs + CI exist to gate *shipping*; changes to `docs/**`, any `*.md`, or `.claude/**` (skills/hooks) don't deploy and don't need their own CI story. Commit them on the branch **currently being worked on** — they ride along with that branch's PR. Never park them on a standalone branch for a later PR to absorb (rule v1; retired because it had no enforcement — it relied on the next branch remembering to base off it, which failed on day one). If **no** work branch is open, cut a short branch and open + merge its own docs-only PR **immediately** on green CI (the PR #4 pattern) — main stays PR-only and nothing dangles. `deploy.yml` `paths-ignore`s those paths, so docs-only merges never trigger a prod deploy.
