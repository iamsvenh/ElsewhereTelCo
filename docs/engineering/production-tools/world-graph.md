...
- **Phase 1 — cheap, high overview payoff.** Auto-render a **read-only** map from the same files, served gated by the bridge. **Start with generated Mermaid/Graphviz** (a build script emitting a diagram from the JSON — near-zero code, renders in our web stack and even in markdown). Upgrade to **React Flow** only if interactive pan/zoom/inspect earns it. Add the `node_config` overlay for deploy-free live tuning. **This gives you the visual overview you're losing, now, cheaply.**
- **Phase 2 — only when a trigger fires.** In-canvas editing that writes **drafts to the DB overlay** + a **"promote to git" that opens a PR** — so git stays the ratification gate even with a GUI. This is the incremental bridge to a real editor without abandoning review.
...
## Open decision for Sven

**Adopt the phased path (Phase 0 + 1 now, editor deferred behind triggers)?** My strong recommendation: yes. It gives you the visual overview and the single source of truth this week, for roughly the cost of the switchboard build you already greenlit, without trading away git's review gate. If you specifically want in-canvas *editing* sooner, we can pull Phase 2 forward — but do it as "edit → DB draft → promote opens a PR," so git stays the ratification gate rather than being abandoned.
