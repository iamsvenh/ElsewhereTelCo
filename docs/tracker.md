...
- 🔴 **World Graph / production map — DESIGNED** (`engineering/production-tools/world-graph.md`). Researched (existing tools · OSS graph libs · data model/validation). Recommendation: source of truth = a git-versioned, `zod`-validated structured graph (worlds/nodes/edges + `node_config` overlay) that the bridge walks; **read-only map now** (Mermaid → React Flow later); full visual DB editor DEFERRED behind explicit triggers (avoids "tool eats the product"; keeps git as the ratification gate). Phase 0 ≈ the switchboard build done schema-first (≈0 extra). Awaiting Sven's greenlight on the phased path
...
6. **World Graph / production map** — adopt the phased path? (Phase 0 = build the switchboard schema-first + `zod` CI gate; Phase 1 = read-only Mermaid map + `node_config` overlay; full editor deferred behind triggers). Or pull the editable editor forward. See `engineering/production-tools/world-graph.md`
